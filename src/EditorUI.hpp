...
public:
    ...
    void updateToolLayer(std::shared_ptr<Layer> layer)
    {
        ...
        for (auto &t : tools)
        {
            if (auto brush = dynamic_cast<BrushTool *>(t.get()))
                brush->setLayer(layer);
            else if (auto fill = dynamic_cast<FillTool *>(t.get()))
                fill->setLayer(layer);
            else if (auto eraser = dynamic_cast<EraserTool *>(t.get()))
                eraser->setLayer(layer);
            else if (auto pipette = dynamic_cast<PipetteTool *>(t.get()))
                pipette->setLayer(layer);
            else if (auto shape = dynamic_cast<ShapeTool *>(t.get()))
                shape->setLayer(layer);
            else if (auto sel = dynamic_cast<SelectionTool *>(t.get()))
                sel->setLayer(layer);
            else if (auto txt = dynamic_cast<TextTool *>(t.get()))
                txt->setLayer(layer);
        }
    }
                         ...
                         std::function<void()> onResizeCanvas)
    {
        ...
        if (ImGui::Button(localization.get("grayscale").c_str(), ImVec2(135, 25)))
        {
            applyFilter<GrayscaleFilter>();
        }
        ImGui::SameLine();
        if (ImGui::Button(localization.get("invert").c_str(), ImVec2(135, 25)))
        {
            applyFilter<InvertFilter>();
        }
        if (ImGui::Button(localization.get("blur").c_str(), ImVec2(135, 25)))
        {
            applyFilter<BlurFilter>();
        }
        ImGui::SameLine();
        if (ImGui::Button(localization.get("sharpen").c_str(), ImVec2(135, 25)))
        {
            applyFilter<SharpenFilter>();
        }
        if (ImGui::Button(localization.get("sepia").c_str(), ImVec2(135, 25)))
        {
            applyFilter<SepiaFilter>();
        }
        ImGui::SameLine();
        if (ImGui::Button(localization.get("bright_plus").c_str(), ImVec2(67, 25)))
        {
            applyFilter<BrightnessFilter>(30);
        }
        ImGui::SameLine();
        if (ImGui::Button(localization.get("bright_minus").c_str(), ImVec2(67, 25)))
        {
            applyFilter<BrightnessFilter>(-30);
        }
                      ...
                      int canvasHeight)
    {
        ...
        if (stepPixels >= 500)
            step = 500;
        else if (stepPixels >= 200)
            step = 200;
        else if (stepPixels >= 100)
            step = 100;
        else if (stepPixels >= 50)
            step = 50;
        else if (stepPixels >= 20)
            step = 20;
        else if (stepPixels >= 10)
            step = 10;
        else if (stepPixels >= 5)
            step = 5;
        else if (stepPixels >= 2)
            step = 2;
        else
            step = 1;
        ...
        for (int x = 0; x <= canvasWidth; x += gridSize)
        {
            sf::Vertex line[] = {
                ...
                sf::Vertex(sf::Vector2f(canvasOffset.x + x * zoomLevel,
                                        canvasOffset.y + canvasHeight * zoomLevel),
                           gridColor)};
            window.draw(line, 2, sf::Lines);
        }
